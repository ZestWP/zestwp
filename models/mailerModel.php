...
class Mailer_Model extends dbZest
{
	...
	public function triggerEmail($post){
		...
		foreach($toSendList as $k=>$v){
			$body = $this->contentClean($content, $list[$k], $signature);
			...
			if($return == 1) $logs['success'][$k] = 1;
			else $logs['error'][$k]=$return;
	...
	public function contentClean($content = '', $list = array(), $signature = ''){
